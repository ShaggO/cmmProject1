function [positions, lengths, times, switches, active] = inverseSim(fps, t_stop, stepBound, rFactor)
...
%        pP = pP + dp;
        ...
        % Time update
        time = time+dt;
        cDt = springTimestep(pP, sp, pM, spK, spX0);
        dt = min(cDt(:));
    end
